export const erc20Transfer = async () => {
  try {
    console.log(
      "Transferring 1 USDC to recipient...",
    );
    ...
    const sessionKeyPrivKey =
      window.localStorage.getItem("sessionPKey");
    console.log(
      "sessionKeyPrivKey",
      sessionKeyPrivKey,
    );
    if (!sessionKeyPrivKey) {
      alert(
        "Session key not found please create session",
      );
      return;
    }
    const sessionSigner = new ethers.Wallet(
      sessionKeyPrivKey,
    );
    ...
    const sessionModule =
      await SessionKeyManagerModule.create({
        moduleAddress:
    ...
    try {
      ...
      throw new Error(
        "invalid token address supplied",
      );
    }
